...
def validate_grounding(result):
    ...
    required_box_keys = {"label", "box_2d", "confidence"}
    for b in result["bounding_boxes"]:
        # Contract requires these 3 keys; Innovation 8 adds optional extras
        # (oriented_box_2d, polygon) which are permitted by the innovation spec.
        assert required_box_keys.issubset(set(b.keys())), (
            f"box is missing required keys {required_box_keys}: {sorted(b.keys())}"
        )
        box2d = b["box_2d"]
